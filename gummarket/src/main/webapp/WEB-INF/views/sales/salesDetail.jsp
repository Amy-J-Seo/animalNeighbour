			...
			<div class="card-header py-3">
				<h2 class="m-0 font-weight-bold text-dark" style="text-align: center;">${list[0].sTitle }
					...
					<c:if test="${session.mId != list[0].mId}">
						<a class="btn btn-danger" href="#" data-toggle="modal" data-target="#reportModal" style="float:right;"><i class="fas fa-bullhorn"></i> 신고하기</a>
					</c:if>
					
					<c:if test="${session.mId == list[0].mId}">
						<button class="btn btn-sm" id="deleteBtn" onclick="confirmDel(${list[0].sNo})"
							style="background-color: rgb(255, 190, 83);  color:rgb(255, 255, 255); float:right;"><i class="far fa-trash-alt"></i> 글 삭제</button>
						<button class="btn btn-sm mr-3" id="updateBtn" onclick="updateSales(${list[0].sNo})"
							style="background-color: rgb(255, 190, 83); color:rgb(255, 255, 255); float:right;"><i class="far fa-edit"></i> 글 수정</button>
					</c:if>
				</h2>
				<p></p>
